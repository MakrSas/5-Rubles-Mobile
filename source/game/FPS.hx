...
class FPS extends Sprite
{
	...
	**/
	...
	{
		fpsText.defaultTextFormat.font = resolveFontName(AssetsPaths.font(newFont));
		_dirtyUpdatePos = true;
	}

	function resolveFontName(path:String):String
	{
		try
		{
			if (Assets.exists(path))
				return Assets.getFont(path)?.fontName ?? flixel.system.FlxAssets.FONT_DEFAULT;
		}
		catch (e)
		{
			#if android
			Log('FPS font fallback: $path', YELLOW);
			#end
		}
		return flixel.system.FlxAssets.FONT_DEFAULT;
	}
	...
	public function new(offsetX:Float = 10, offsetY:Float = 10)
	{
		...
		fpsText.defaultTextFormat = new TextFormat(resolveFontName(AssetsPaths.font("VCR OSD Mono Cyr.ttf")), 12, 0xEEEEEE);
	...
	*/
	...
	#if android
	@:noCompletion static final _htmlTagPattern:EReg = ~/<\/?[^>]+>/g;
	#end
	...
	function updateText()
	{
		...
		updateLines();
		#if android
		fpsText.text = _htmlTagPattern.replace(_text, "");
		#else
