...
class OptionsSubState extends MusicBeatSubstate
{
	...
	var categories:Array<String> = ['VISUAL UI', 'GAMEPLAY' #if mobile, 'MOBILE' #end, 'GRAPHICS', 'CONTROLS' #if DEV_BUILD , "DEV" #end];
	...
	public override function create()
	{
		...
		if (inGame)
		{
			final forcedRestartOptions:Array<String> = [
				"mobileMenuButtons",
				"vSliceControls"
			];
			function a(datas:Array<SaveOption>)
			{
				...
				for(data in datas)
				{
					if (((data.changeInGameplay != true && data.typeVar != INPUT)
						|| forcedRestartOptions.contains(data.variableName)))
