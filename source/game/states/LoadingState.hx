...
class LoadingState extends MusicBeatSubstate
{
		...
		if (startedLoading)
		{
			while (funcsPrepare.length > 0 && currentUsedThread < maxThreadsAllowed)
			{
				...
				if (FlxG.onMobile)
				{
					...
					{
						Log('Failed load assets: $e', RED);
					}
					currentUsedThread--;
					dirtyUpdateGraphics = true;
					// Process one heavy task per frame on mobile to avoid long stalls/ANR.
					break;
				}
	...
	public function loadAtlasAlt(path:String)
	{
		...
		callback(() -> {
			...
			try
			{
				atlasLoaded = Paths.getAtlas(noExt) != null;
			}
			catch (e)
			{
				Log('Failed to parse atlas "$noExt": $e', RED);
			}

			if (!atlasLoaded)
			{
				var imagePath = '$noExt.png';
				if (Assets.exists(imagePath))
					loadImageAlt(imagePath, true);
			}
			/*
