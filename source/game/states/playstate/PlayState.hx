...
class PlayState extends MusicBeatState {
	...
	#if TOUCH_CONTROLS
	var mobilePauseButton:FunkinSprite;
	var mobilePauseTouchPoint:FlxPoint = FlxPoint.get();
	var mobileStrumTouchPoint:FlxPoint = FlxPoint.get();
	var mobileStrumTouchState:Array<Bool> = [];
	#end
	...
	public override function create(){
		...
		*/
		...
		if (hitbox != null && isMobileVSliceControlsEnabled())
		{
			hitbox.active = false;
		...
		final callbacks:Array<LoadingState.LoadingStateCallback> = [
			...
			addCallback.bind(_, "idk", () -> {
				...
				// РЎРѓ Р Р…Р С•РЎвЂљР В°Р СР С‘ Р С—РЎР‚Р С‘Р Т‘РЎвЂРЎвЂљРЎРѓРЎРЏ Р ВµР В±Р В°РЎвЂљРЎРЉРЎРѓРЎРЏ, РЎРЊРЎвЂ¦ СЂСџВСћ
		...
		/*
		else // TODO: Fix openfl issue with function 'getMatriРЎРѓes'
		{
	...
	public function setupStrumLine(strumLine:StrumLine)
	{
		...
		#if NOTE_BACKWARD_COMPATIBILITY
			strumLine.onSpawnNote.add(note -> {
				if (note != null && notes != null)
					notes.insert(0, note);
			});
			strumLine.onSpawnNote.add(note -> {
				if (note != null && unspawnNotes != null)
					unspawnNotes.remove(note);
			});
			strumLine.onDestroyNote.add(note -> {
				if (note != null && notes != null)
	...
	{
		if (FlxG.onMobile && !ClientPrefs.mobileSustainReleaseMiss
			&& daNote.mustPress && daNote.isSustainNote
			&& daNote.parent != null && daNote.parent.wasGoodHit)
		{
			daNote.ignoreNote = true;
			strumLine.destroyNote(daNote, true);
			return;
		}
	...
	#if TOUCH_CONTROLS
	inline function useMobilePauseButtonForCurrentSong():Bool
	{
	...
	function createMobilePauseButton()
	{
		...
		final graphic = Paths.image("mobileUI/pause");
		if (graphic == null)
			return;

		mobilePauseButton = new FunkinSprite();
		mobilePauseButton.loadGraphic(graphic);
		mobilePauseButton.antialiasing = ClientPrefs.globalAntialiasing;
		mobilePauseButton.scrollFactor.set();
		mobilePauseButton.cameras = [camControls];
		mobilePauseButton.alpha = 0.85;
		mobilePauseButton.visible = false;

		final targetSize:Int = Std.int(Math.min(FlxG.width, FlxG.height) * 0.115);
		if (targetSize > 0)
			mobilePauseButton.setGraphicSize(targetSize, targetSize);
		mobilePauseButton.updateHitbox();

		final margin:Float = Math.max(12, targetSize * 0.2);
		mobilePauseButton.setPosition(FlxG.width - mobilePauseButton.width - margin, margin);
		...
		if (hitbox != null && hitbox.active)
		{
			hitbox.forEach((hint:MobileHint) -> {
				hint.ignoreZone.push(mobilePauseButton);
			});
		}
	}

	function mobilePausePressed():Bool
	{
		if (!FlxG.onMobile || mobilePauseButton == null || !mobilePauseButton.visible)
			return false;
		...
		for (touch in FlxG.touches.list)
		{
			...
			if (mobilePauseButton.overlapsPoint(touch.getWorldPosition(camControls, mobilePauseTouchPoint), true, camControls))
			{
				GamepadUtil.vibrateButton();
				return true;
			}
		}
	...
	function startSong():Void
	{
		...
		if (hitbox != null)
		{
			final showTouchHitbox:Bool = FlxG.onMobile && !isMobileVSliceControlsEnabled() && !suppressFallbackMobileHitboxForCurrentSong() && !cpuControlled;
			hitbox.active = showTouchHitbox;
			hitbox.visible = showTouchHitbox;
		}
		if (mobilePauseButton != null) mobilePauseButton.visible = useMobilePauseButtonForCurrentSong();
		#end
	...
	inline function isJopotr4ssSong():Bool
	{
		return songName != null && songName.indexOf('jopotr4ss') == 0;
	}

	inline function isMatematikaSong():Bool
	{
		return songName == 'matematika';
	}

	inline function isBezdariFamilySong():Bool
	{
		return songName != null && songName.indexOf('bezdari') == 0;
	}

	inline function isVSliceSpacingRefreshSong():Bool
	{
		return isBezdariFamilySong() || songName == 'add-me-in-5rubles';
	}

	inline function forceShowOpponentStrumsForCurrentSong():Bool
	{
		#if TOUCH_CONTROLS
		return FlxG.onMobile && songName == 'hernyamorumix';
		#else
		return false;
		#end
	}

	inline function hideOpponentStrumsForCurrentSong():Bool
	{
		#if TOUCH_CONTROLS
		return FlxG.onMobile && isMatematikaSong();
		#else
		return false;
		#end
	}
	inline function hideExtraMobileStrumlinesForCurrentSong():Bool
	{
		#if TOUCH_CONTROLS
		if (!FlxG.onMobile)
			return false;

		return songName == 'bezdari'
			|| songName == 'bezdariamalgamatmix'
			|| songName == 'bezdarinaisonjimix'
			|| songName == 'add-me-in-5rubles';
		#else
		return false;
		#end
	}
	inline function suppressFallbackMobileHitboxForCurrentSong():Bool
	{
		return false;
	}

	inline function isMobileVSliceControlsEnabled():Bool
	{
		#if TOUCH_CONTROLS
		if (!FlxG.onMobile || !ClientPrefs.vSliceControls)
			return false;


		// This song has its own bespoke strum/touch pipeline.
		if (isJopotr4ssSong())
			return false;
	...
	function applyMobileVSliceStrumLayout(strums:StrumGroup):Void
	{
		if (!isMobileVSliceControlsEnabled() || strums == null)
			return;

		final count:Int = strums.members.length;
		if (count <= 1)
			return;
		...
		for (i => strum in strums.members)
		{
			...
			final laneX:Float = startX + i * (laneSize + spacing);
			final targetX:Float = laneX + (laneSize - strum.width) * 0.5;
			strum.x = strum.defPos.x = targetX;
		}
	}
	inline function forceHideOpponentStrumlineForMobileVSlice():Bool
	{
		#if TOUCH_CONTROLS
		return isMobileVSliceControlsEnabled();
		#else
		return false;
		#end
	}

	function applyForcedMobileVSliceStrumlineVisibility():Void
	{
		#if TOUCH_CONTROLS
		final forceHide:Bool = forceHideOpponentStrumlineForMobileVSlice();
		final forceShowOpponent:Bool = forceShowOpponentStrumsForCurrentSong();
		final forceHideOpponent:Bool = hideOpponentStrumsForCurrentSong();
		final hideExtraLines:Bool = hideExtraMobileStrumlinesForCurrentSong();
		if (!forceHide && !hideExtraLines && opponentStrumLine == null && hitbox == null)
			return;

		if (hitbox != null)
		{
			final showTouchHitbox:Bool = FlxG.onMobile && startedCountdown && !endingSong && !paused && !cpuControlled && !forceHide && !suppressFallbackMobileHitboxForCurrentSong();
			hitbox.active = showTouchHitbox;
			hitbox.visible = showTouchHitbox;
		}
		...
		for (line in strumLines)
		{
			if (line == null || line.strumNotes == null)
				continue;

			final isOpponentLine:Bool = (line == opponentStrumLine);
			final isExtraLine:Bool = !line.isPlayer && !isOpponentLine;
			if (!isOpponentLine && !(hideExtraLines && isExtraLine))
				continue;

			final hideLine:Bool = (isOpponentLine && (forceHideOpponent || (forceHide && !forceShowOpponent))) || (hideExtraLines && isExtraLine);
			final showLine:Bool = isOpponentLine
				? ((forceShowOpponent && isOpponentLine) || (!hideLine && ClientPrefs.opponentStrums))
				: !hideLine;
			for (strum in line.strumNotes.members)
			{
				if (strum == null)
					continue;
				strum.visible = showLine;
				if (!showLine)
					strum.alpha = 0;
				else if (strum.alpha <= 0)
					strum.alpha = 1;
			}
		}
		#end
	...
	private function generateStaticArrows(player:Bool):Void
	{
		...
		if (player)
		{
			...
			applyMobileVSliceStrumLayout(playerStrums);
			applyForcedMobileVSliceStrumlineVisibility();
		if (isMobileVSliceControlsEnabled() && isVSliceSpacingRefreshSong())
			applyMobileVSliceStrumLayout(playerStrums);
		}
		else
		{
			...
			{
				strumLineNotes.add(i);
				i.visible = !hideOpponentStrumsForCurrentSong() && (forceShowOpponentStrumsForCurrentSong() || (!forceHideOpponentStrumlineForMobileVSlice() && ClientPrefs.opponentStrums));
				i.playAnim('static');
			}
			if (middleScrollMode)
			{
	...
	*/
	...
	function setPause(toogle:Bool)
	{
		...
		else
		{
			...
			if (mobilePauseButton != null)
				mobilePauseButton.visible = useMobilePauseButtonForCurrentSong() && startedCountdown && canPause && !endingSong;
			if (hitbox != null)
			{
				final showTouchHitbox:Bool = FlxG.onMobile && startedCountdown && !endingSong && !cpuControlled && !isMobileVSliceControlsEnabled() && !suppressFallbackMobileHitboxForCurrentSong();
				hitbox.active = showTouchHitbox;
				hitbox.visible = showTouchHitbox;
			}
			#end
	...
	{
		#if TOUCH_CONTROLS
		if (mobilePauseButton != null) mobilePauseButton.visible = false;
		if (hitbox != null)
		{
			hitbox.active = false;
			hitbox.visible = false;
	...
	{
		var pausePressed:Bool = controls.PAUSE;
		#if TOUCH_CONTROLS
		pausePressed = pausePressed || mobilePausePressed() || (FlxG.onMobile && controls.BACK);
		#end

		#if DEV_BUILD
		if (isFreezed)
		{
			if (pausePressed && canPause && startedCountdown && !ScriptPack.resultIsStop(callOnScripts('onPause', null, false)))
				openPauseMenu();
	...
	function keyShit():Void
	{
		...
		/*if (replayMode){
			...
			#if TOUCH_CONTROLS
			if (isMobileVSliceControlsEnabled() && !cpuControlled && playerStrums != null)
			{
				while (mobileStrumTouchState.length < keysArray.length)
					mobileStrumTouchState.push(false);

				final touchHolding:Array<Bool> = [for (_ in 0...keysArray.length) false];
				...
				for (touch in FlxG.touches.list)
				{
					...
					if (mobilePauseButton != null && mobilePauseButton.visible
						&& mobilePauseButton.overlapsPoint(touch.getWorldPosition(camControls, mobilePauseTouchPoint), true, camControls))
						continue;

					final touchPos = touch.getWorldPosition(camHUD, mobileStrumTouchPoint);

					// If touch is exactly between two lanes, press both adjacent arrows.
					...
					for (lane in 0...keysArray.length - 1)
					{
						...
						var centerLeft:Float = leftStrum.x + leftStrum.width * 0.5;
						var centerRight:Float = rightStrum.x + rightStrum.width * 0.5;
						if (centerLeft > centerRight)
						{
							final tmp:Float = centerLeft;
						...
						{
							touchHolding[lane] = true;
							touchHolding[lane + 1] = true;
							pressedGapPair = true;
							break;
						}
					}
					...
					var bestLane:Int = -1;
					var bestLaneDistance:Float = 1e20;
					for (i in 0...keysArray.length)
					{
						final strum:StrumNote = playerStrums.members[i];
						...
						if (touchPos.x >= left && touchPos.x <= right && touchPos.y >= top && touchPos.y <= bottom)
						{
							...
							final laneDistance:Float = Math.abs(touchPos.x - laneCenterX);
					...
					if (bestLane >= 0)
						touchHolding[bestLane] = true;
				}

				for (i in 0...keysArray.length)
				{
					final wasHolding:Bool = mobileStrumTouchState[i];
					final nowHolding:Bool = touchHolding[i];
					holdArray[i] = nowHolding;
					pressArray[i] = nowHolding && !wasHolding;
					releaseArray[i] = !nowHolding && wasHolding;
					mobileStrumTouchState[i] = nowHolding;
				}
			}
			else
			{
				for (i in 0...mobileStrumTouchState.length)
					mobileStrumTouchState[i] = false;
			}
			#end
		}
	...
	function noteMiss(daNote:Note, strumLine:StrumLine, removeNote:Bool = true, ?char:Character):Void{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (FlxG.onMobile && !cpuControlled && daNote.mustPress) GamepadUtil.vibrateMiss();
	...
	function noteMissPress(direction:Int = 1):Void // You pressed a key when there was no notes to press for this key
	{
		...
		if (FlxG.onMobile && !cpuControlled) GamepadUtil.vibrateMiss();
		/*
			boyfriend.stunned = true;
	...
	function goodNoteHit(note:Note, strumLine:StrumLine):Void
	{
		...
		if (FlxG.onMobile && !cpuControlled && note.mustPress)
		{
			if (note.isSustainNote)
				GamepadUtil.vibrateSustainTap();
			else
	...
	*/
	...
	override function destroy()
	{
		...
		*/
		...
		mobilePauseTouchPoint.put();
		mobileStrumTouchPoint.put();
		mobileStrumTouchState = [];
		#end
		super.destroy();
