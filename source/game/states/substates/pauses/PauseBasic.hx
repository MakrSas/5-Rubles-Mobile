...
class PauseBasic extends PauseSubState
{
	...
	function mobileButtonPressed(button:FlxSprite):Bool
	{
		...
		for (touch in FlxG.touches.list)
		{
			if (!touch.pressed)
				continue;
			if (button.overlapsPoint(touch.getWorldPosition(camera, mobileTouchPoint), true, camera))
				return true;
		}
		return false;
	}
	...
	*/
	...
	{
		#if TOUCH_CONTROLS
		if (FlxG.onMobile && useMobileMenuButtons())
		{
	...
	override function update(elapsed:Float)
	{
		...
		if (subState == null && !itIsAccepted)
		{
			...
			final mobileUpP:Bool = usingMobileButtons && mobileButtonJustPressed(mobileButtonUp);
			final mobileDownP:Bool = usingMobileButtons && mobileButtonJustPressed(mobileButtonDown);
			final mobileEnterP:Bool = usingMobileButtons && mobileButtonJustPressed(mobileButtonEnter);
			#else
			...
			if (menuItems[curSelected] == 'Skip Time')
			{
				if (controls.UI_LEFT || controls.UI_RIGHT)
				{
					...
					if (holdTime > 0.5)
					{
						var holdDirection:Int = 1;
						if (controls.UI_LEFT)
							holdDirection = -1;
						curTime += 45000 * elapsed * holdDirection;
					}
			...
			updateSkipTextStuff();
			if (controls.UI_UP_P || (!usingMobileButtons && SwipeUtil.swipeUp)
				#if TOUCH_CONTROLS
				|| mobileUpP
				#end
			)
			...
			if (controls.UI_DOWN_P || (!usingMobileButtons && SwipeUtil.swipeDown)
				#if TOUCH_CONTROLS
				|| mobileDownP
				#end
			)
			...
			if (!usingMobileButtons && TouchUtil.justReleased && !SwipeUtil.swipeAny)
			{
				final touch = TouchUtil.touch;
				touchAccept = touch != null && touch.x < FlxG.width * 0.5;
			}
			if ((controls.ACCEPT || touchAccept
				#if TOUCH_CONTROLS
				|| mobileEnterP
				#end
			) && cantUnpause <= 0) onAccept(menuItems[curSelected]);
		}
