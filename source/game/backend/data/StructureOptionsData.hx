...
class StructureOptionsData
{
	...
	public static var data:Map<String, Array<SaveOption>> = [
        ...
        "MOBILE" => [
			{
				display: "MOBILE MENU BUTTONS",
				variableName: "mobileMenuButtons",
				description: "On mobile, disables swipe gestures in menus and pause and replaces them with on-screen directional buttons and Enter.",
				checkVisible: () -> FlxG.onMobile,
				changeInGameplay: true
			},
			{
				display: "V-SLICE CONTROLS",
				variableName: "vSliceControls",
				description: "Use V-Slice style touch strums in gameplay.",
				checkVisible: () -> FlxG.onMobile,
				changeInGameplay: true
			},
			{
				display: "LONG NOTE RELEASE MISS",
				variableName: "mobileSustainReleaseMiss",
				description: "If OFF, releasing a hold note early on mobile won't count as a miss.",
				checkVisible: () -> FlxG.onMobile,
				changeInGameplay: true
			},
			{
				display: "GAMEPLAY VIBRATION",
				variableName: "mobileGameplayVibration",
				description: "Vibration strength for successful arrow hits.",
				checkVisible: () -> FlxG.onMobile,
				minValue: 0,
				maxValue: 1,
				decimals: 2,
			...
			{
				display: "MISS VIBRATION",
				variableName: "mobileMissVibration",
				description: "Vibration strength for misses.",
				checkVisible: () -> FlxG.onMobile,
				minValue: 0,
				maxValue: 1,
				decimals: 2,
			...
			{
				display: "BUTTON VIBRATION",
				variableName: "mobileButtonVibration",
				description: "Vibration strength for touch UI buttons.",
				checkVisible: () -> FlxG.onMobile,
				minValue: 0,
				maxValue: 1,
				decimals: 2,
		...
		"GRAPHICS" => [
			...
			*/
			...
			// 	changeInGameplay: true,
			// 	description: 'If checked, the game is drawn at the current screen frequency, but retains the frequency from the вЂњFRAMERATEвЂќ option to update the game logic.',
			// }
			// #end
		],
		"CONTROLS" => [
	...
	public static function filterOption(optionData:SaveOption)
	{
		...
		optionData.displayFormat ??= (optionData.typeVar == PERCENT ? "%v %" : "%v"); // СЃРјРµС€РЅСЏРІРєР°
		return optionData;
	}
