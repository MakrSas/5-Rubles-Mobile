...
class Main extends flixel.FlxGame
{
	...
	{
		function addDefaultLibrary(lib:lime.utils.AssetLibrary)
		{
			if (lib != null && !AssetsPaths.assetsTree.__defaultLibraries.contains(lib))
				AssetsPaths.assetsTree.__defaultLibraries.push(lib);
		}
		...
		if (LimeAssets.libraries.exists("assets"))
		{
			var assetsLib = LimeAssets.libraries.get("assets");
			addDefaultLibrary(assetsLib);
			if (assetsLib != null)
				LimeAssets.libraries.remove("assets");
		}

		if (LimeAssets.libraries.exists("embed"))
		{
			var embedLib = LimeAssets.libraries.get("embed");
			addDefaultLibrary(embedLib);
			if (embedLib != null)
		...
		for (path in AssetsMacro.getSecretZipFilesMacro())
		{
			...
			try
			{
				addDefaultLibrary(ModsFolder.loadLibraryFromZip('hxzjbcjhxzbc' + _i++, "./" + path, true));
			}
			catch(e)
			{
				Sys.exit(1);
		...
		addDefaultLibrary(ModsFolder.prepareModLibrary(libName, lib, true));
		#else
		addDefaultLibrary(ModsFolder.loadLibraryFromFolder('assets', "./assets/", true));
		#end
		#end
	...
	{
		var fontPath = AssetsPaths.font('engine/${fontName ?? "Minecraft Rus.ttf"}');
		try
		{
			if (Assets.exists(fontPath))
				flixel.system.FlxAssets.FONT_DEFAULT = Assets.getFont(fontPath)?.fontName ?? flixel.system.FlxAssets.FONT_DEFAULT;
		}
		catch (e)
		{
			#if android
			Log('Failed to load default font: $fontPath', YELLOW);
			#end
		}
	}
