...
class StrumLine extends FlxGroup
{
	...
	#if android
	inline function safeDispatchSpawnNote(note:Note):Void
	{
		try
		{
			onSpawnNote.dispatch(note);
		}
		catch (e)
		{
			trace('[StrumLine] onSpawnNote dispatch failed: ${Std.string(e)}');
		}
	}

	inline function safeDispatchUpdateNote(note:Note, time:Float, speed:Float, timeScale:Float):Void
	{
		try
		{
			onUpdateNote.dispatch(note, time, speed, timeScale);
		}
		catch (e)
		{
			trace('[StrumLine] onUpdateNote dispatch failed: ${Std.string(e)}');
		}
	}

	inline function safeDispatchMissNote(note:Note):Void
	{
		try
		{
			onMissNote.dispatch(note);
		}
		catch (e)
		{
			trace('[StrumLine] onMissNote dispatch failed: ${Std.string(e)}');
		}
	}

	inline function safeDispatchDestroyNote(note:Note):Void
	{
		try
		{
			if (onDestroyNote != null)
				onDestroyNote.dispatch(note);
		}
		catch (e)
		{
			trace('[StrumLine] onDestroyNote dispatch failed: ${Std.string(e)}');
		}
	}
	#end
	...
	public function updateNotesAtTime(elapsed:Float, time:Float, localSpeed:Float = 1, timeScale:Float = 1, updateLogic:Bool = true)
	{
		...
		if (unspawnNotes.length != 0)
		{
			...
			while (dunceNote != null && dunceNote.strumTime - time < spawnTime / dunceNote.multSpeed)
			{
				...
				#if android
				if (dunceNote.parentStrum == null)
				{
					var dataIndex:Int = dunceNote.noteDataReal;
					final strumCount = strumNotes.members.length;
					if (strumCount > 0)
					{
						if (dataIndex < 0 || dataIndex >= strumCount)
							dataIndex = Std.int(Math.abs(dataIndex) % strumCount);
						dunceNote.parentStrum = strumNotes.members[dataIndex];
					}
				}
				if (dunceNote.parentStrum == null)
				{
					trace('[StrumLine] Missing parent strum, skipped note ${dunceNote.noteDataReal} (${isPlayer ? "player" : "opponent"})');
					dunceNote.kill();
					destroyNote(dunceNote, true);
					dunceNote = unspawnNotes[0];
					continue;
				}
				safeDispatchSpawnNote(dunceNote);
				#else
		...
		for (daNote in spawnedNotes)
		{
			...
			#if android
			safeDispatchUpdateNote(daNote, time, speed, timeScale);
			#else
	...
	public function destroyNote(daNote:Note, force:Bool = false){
		...
		if (!force && !daNote.destroyOnHit)
			return false;
		#if android
		safeDispatchDestroyNote(daNote);
		#else
