...
function applyMatematikaVSliceSpacing()
{
	if (!FlxG.onMobile || !ClientPrefs.vSliceControls || playerStrums == null || playerStrums.members == null)
		return;

	final count = playerStrums.members.length;
	if (count <= 1)
		return;

	final minSize = Math.min(FlxG.width, FlxG.height);
	var laneSize = Math.max(64.0, minSize * 0.15);
	var spacing = Math.max(24.0, laneSize * 0.85);
	final maxSpacing = (FlxG.width - laneSize * count) / (count - 1);
	if (maxSpacing < spacing)
		spacing = Math.max(8.0, maxSpacing);

	final totalWidth = laneSize * count + spacing * (count - 1);
	final startX = (FlxG.width - totalWidth) * 0.5;
	...
	for (i => strum in playerStrums.members)
	{
		...
		final laneX = startX + i * (laneSize + spacing);
		final targetX = laneX + (laneSize - strum.width) * 0.5;
	...
	switch (name)
	{
		case "":
			...
			switch (eventValue)
			{
				...
				case "sleepybox" | "WAKEY WAKEY" | "wakey wakey":
					redarStrumsSleepToo(eventValue.toLowerCase() == "sleepybox");

				case "Alt Idle" | "alt idle":
					var beat = 2;
					var suffix = "";
				...
				case "Rich Time" | "rich time": // богатое время
					richTime = true;
...
function onUpdatePost(elapsed:Float)
{
	...
	var target = (_camTarget == null ? "" : Std.string(_camTarget)).trim().toLowerCase();
	onChangeFocusChar(switch (target)
	{
...
function onChangeFocusChar(char:Character)
{
	...
	if (newIcon == null)
	{
		newIcon = new HealthIcon(char.healthIcon, isPlayer);
		newIcon.y = (isPlayer ? iconP1 : iconP2).y;
		newIcon.alpha = 0.0;
		pushIcon(newIcon);
	}
...
function redarStrumsSleepToo(toSleep:Bool = true)
{
	if (toSleep)
		...
		{
			FlxTween.cancelTweensOf(strummy);
			FlxTween.tween(strummy, {x: strummy.x + FlxG.random.float(-120, 120), y: strummy.y + FlxG.random.float(95, 105), angle: FlxG.random.float(-16, 16), alpha: .5}, 9, {ease:FlxEase.sineIn, startDelay: FlxG.random.float(.0, .25)});
		}
	else
		for (i => strummy in playerStrums.members)
		{
			FlxTween.cancelTweensOf(strummy);
			FlxTween.tween(strummy, {x: strummy.defPos.x, y: playerStrums.position.y, angle: 0, alpha: 1}, .4, {ease:FlxEase.backOut, startDelay: FlxG.random.float(0, .1)});
		}
}
