...
import game.mobile.scaleModes.JopaScaleMode;
...
function onCreate()
{
	...
	FlxG.scaleMode = new JopaScaleMode();
	#end
...
function onCreatePost()
{
	...
	jopotr4ssSpine.setPosition(boyfriend.x, boyfriend.y);
...
function goodNoteHit(note:Note)
{
	...
	#if ALLOW_HAPTICS
	// ВИБРАЦИЯ ШЛЕПКОВ
	Haptic.vibrateOneShot(getSlapPower() / 8, getSlapPower() / 2);
	#end
...
{
	// тут была отсалка на кепочку мисайд, сейчас она пропала
