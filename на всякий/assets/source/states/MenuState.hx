			...
			switchFunc: () ->
			{
				...
				new FlxTimer().start(1, _ ->
				{
					...
					FlxG.state.openSubState(new FreeplayState(() ->
					{
						...
						tomacoolSound.resume();
					}));
				});
				var selected = menuItems[curRow][curSelected];
				if (ClientPrefs.flashing)
					FlxFlicker.flicker(selected, 1, 0.075, true, true);
			},
...
function create()
{
	...
	if (isShowingTexts)
	{
		...
		new FlxTimer().start(1, _ ->
		{
			var isValidNewUpdate = __globalScript__.getVar			("updateData") != null;
			if (isValidNewUpdate)
				selectedSomethin = true;
				...
				if (timer.kill)
				{
					...
					if (isValidNewUpdate)
					{
						new FlxTimer().start(0.8, _ -> {
							selectedSomethin = false;
							openSubState(new MusicBeatSubstate(FlxColor.TRANSPARENT, true, "UpdateSubState"));
						});
					}
