...
function create()
{
	...
	defaultStatsText.setFormat(Paths.font('comicbd.ttf'), 20, 0xffffffff, "right");
	add(defaultStatsText);

	advice = new FlxFixedText(0, 0, 0, "huh?");
	advice.setFormat(Paths.font('comicbd.ttf'), 23, 0xffffffff, "center");
	add(advice);
	reloadAdvice();
...
function preUpdate(elapsed:Float)
{
	...
	if (defaultStatsText != null)
	{
		...
		if (daText != defaultStatsText.text)
		{
			...
			defaultStatsText.x = FlxG.width - defaultStatsText.width - 20;
